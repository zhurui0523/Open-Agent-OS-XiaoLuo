...
import { modelById } from "./manifest.mjs";
...
/** 用户自定义添加的本地模型（存于 config.json customModels，仅注册信息，文件位置由用户提供） */
export function customModels() {
  const list = readConfig().customModels;
  return Array.isArray(list) ? list : [];
}

/** 统一模型查找：内置清单 ∪ 自定义模型 */
export function resolveModel(modelId) {
  return modelById(modelId) || customModels().find((model) => model.id === modelId) || null;
}
