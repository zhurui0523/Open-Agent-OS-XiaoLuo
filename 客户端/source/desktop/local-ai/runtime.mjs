...
import { mutateState, readConfig, readState, resolveModel } from "./state.mjs";
import { detectHardware } from "./hardware.mjs";
import { modelFiles, primaryFileName } from "./manifest.mjs";
...
export async function startModel(modelId) {
  ...
  const binary = binaryFor(kind);
  // 自定义模型：文件在用户指定的绝对路径；内置模型：存储目录内按清单校验
  const modelPath = model.filePath || join(storagePath, primaryFileName(model));
  if (binary.kind === "real") {
    if (model.filePath) {
      if (!existsSync(model.filePath)) throw new Error("模型文件不存在：" + model.filePath);
    } else {
      for (const file of modelFiles(model)) {
  ...
  if (binary.kind === "real" && kind === "text") {
    args = ["--model", modelPath, "--host", "127.0.0.1", "--port", String(port), "--ctx-size", String(model.context || 8192), "--n-gpu-layers", gpuLayersFor(hardware.tier), "--alias", modelId];
