...
if (-not (Test-Path -LiteralPath $mysqld)) {
  if (-not (Test-Path -LiteralPath $archive)) {
    Write-Host "[XiaoLuo AI] Downloading MySQL $MySqlVersion LTS from Oracle..."
  ...
  Write-Host "[XiaoLuo AI] Extracting local MySQL..."
...
if (-not (Test-Path -LiteralPath (Join-Path $dataRoot "mysql"))) {
  ...
  New-Item -ItemType Directory -Force -Path $dataRoot | Out-Null
  Write-Host "[XiaoLuo AI] Initializing local MySQL data directory..."
...
Write-Host "[XiaoLuo AI] Starting local MySQL..."
...
Write-Host "[XiaoLuo AI] Local MySQL is ready on 127.0.0.1:3307."
