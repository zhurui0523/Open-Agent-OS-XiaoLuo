...
try {
  ...
  const [users] = await connection.execute(
    "SELECT id FROM xiaoluo_v2_users WHERE email = ? OR phone_hash = ? LIMIT 1",
    [email, phoneHash],
  );
  ...
  if (users.length) {
    await connection.execute(
      `UPDATE xiaoluo_v2_users
       ...
       WHERE id = ?`,
      [email, displayName, passwordHash(password), phoneHash, phone.slice(-4), id],
    );
  } else {
    await connection.execute(
      `INSERT INTO xiaoluo_v2_users
        (
          id, email, display_name, password_hash, phone_hash, phone_last4,
          phone_verified_at, platform_role, status
        )
       VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP(3), 'system_admin', 'active')`,
      [id, email, displayName, passwordHash(password), phoneHash, phone.slice(-4)],
    );
  }
