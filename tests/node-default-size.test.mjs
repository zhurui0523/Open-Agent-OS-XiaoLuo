...
test("execution and text result nodes use fixed default sizes with internal scrolling", async () => {
  ...
  assert.match(layout, /EXECUTION_NODE_WIDTH = 360/);
  assert.match(layout, /EXECUTION_NODE_HEIGHT = 300/);
  ...
  assert.match(layout, /if \(role === "execution"\) return EXECUTION_NODE_WIDTH/);
  assert.match(nodeCard, /isExecutionNode/);
  assert.match(nodeCard, /isExecutionNode[\s\S]{0,100}EXECUTION_NODE_WIDTH/);
  assert.match(nodeCard, /isExecutionNode[\s\S]{0,100}EXECUTION_NODE_HEIGHT/);
  assert.match(
    styles,
  ...
  assert.match(canvasView, /center\.y - EXECUTION_NODE_HEIGHT \/ 2/);
});
