...
import { brainWorkspaceRoot, cancelSandboxedCommands, checkPathFence, classifyCommandRisk, runSandboxedCommand } from "../../../../lib/brain-sandbox";
...
export async function DELETE(request: Request) {
  try {
    ...
    cancelSandboxedCommands();
    return Response.json({ ok: true });
  } catch (error) {
    return jsonError(error, "取消命令失败");
  }
}