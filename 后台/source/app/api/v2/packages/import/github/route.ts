...
import { uploadPackageContentToOss } from "../../../../../lib/package-oss";
import {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // PACKAGE-OSS：包体以远程 OSS 为事实源（本地磁盘只是可再生执行缓存）
    const inspectionSnapshot = inspection;
    if (inspectionSnapshot) {
      await uploadPackageContentToOss({
        packageKey: manifest.id,
        version: manifest.version,
        manifestJson: canonicalPackageManifest(manifest),
        permissionsJson: JSON.stringify(manifest.permissions ?? []),
        files: inspectionSnapshot.files.map((file) => ({
          relativePath: file.relativePath,
          read: () => inspectionSnapshot.readFile(file.path),
        })),
      });
    }
