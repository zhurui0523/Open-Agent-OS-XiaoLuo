...
import { ensurePackageMaterialized } from "../../../../lib/package-oss";
...
export async function POST(request: Request) {
  try {
    ...
    // PACKAGE-OSS：执行前按需从 OSS 物化包体（本地 PACKAGE_ROOT 为可再生缓存）
    const materialized = await ensurePackageMaterialized({
      packageKey: pkg.packageKey,
      version: pkg.version,
      integritySha256: pkg.integritySha256,
    });
    if (!materialized.prepared) {
      return Response.json(
        { error: "Package 包体不在 OSS，无法执行；请重新安装该 Package" },
        { status: 409 },
      );
    }
