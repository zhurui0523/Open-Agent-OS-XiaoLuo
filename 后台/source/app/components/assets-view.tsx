...
import { HttpProgramStore } from "../xiaoluo-brain/lib/brain/api";
import type { ProgramMeta, ProgramRecord } from "../xiaoluo-brain/lib/brain/types";

const filters: Array<{ id: "all" | "programs" | AssetKind; label: string }> = [
  { id: "all", label: "全部" },
  { id: "image", label: "图片" },
  { id: "video", label: "视频" },
  { id: "audio", label: "音频" },
...
export function AssetsView({
  ...
  const [filter, setFilter] = useState<"all" | "programs" | AssetKind>("all");
  ...
  const loadAssets = useCallback(async (cursor?: string) => {
    // 程序库页签走独立数据源（/api/v2/brain/programs），不请求文件索引
    if (filter === "programs") {
      setAssets([]);
      setNextCursor(null);
      setLoading(false);
  ...
  // ---------- 程序库页签（原大脑面板右上角入口搬到文件系统） ----------
  const programStore = useMemo(() => new HttpProgramStore({ base: "" }), []);
  const [programs, setPrograms] = useState<ProgramMeta[]>([]);
  const [programsNonce, setProgramsNonce] = useState(0);
  const [programsLoading, setProgramsLoading] = useState(false);
  const [programDetail, setProgramDetail] = useState<ProgramRecord | null>(null);

  useEffect(() => {
    if (filter !== "programs") return;
    let live = true;
  ...
  }, [filter, programsNonce, programStore]);

  async function renameProgram(prog: ProgramMeta) {
    const next = await dialog.prompt("修改程序的显示名称。", {
      title: "重命名程序",
      inputLabel: "程序名称",
      defaultValue: prog.name,
      confirmText: "保存名称",
    });
    if (!next?.trim() || next.trim() === prog.name) return;
    await programStore.rename(prog.id, next.trim());
    setProgramsNonce((v) => v + 1);
  }

  async function removeProgram(prog: ProgramMeta) {
    const ok = await dialog.confirm("程序「" + prog.name + "」将被永久删除，此操作不能撤销。", {
      title: "删除程序",
      confirmText: "永久删除",
      tone: "danger",
    });
    if (!ok) return;
    await programStore.remove(prog.id);
    setProgramsNonce((v) => v + 1);
  }

  function openProgramDetail(prog: ProgramMeta) {
    void programStore.get(prog.id).then((rec) => {
      if (rec) setProgramDetail(rec);
    });
  }
  ...
  return (
    ...
    >
      ...
      <div className="file-system-stats">
        <span><HardDrive size={15} /> {filter === "programs" ? "程序库 " + programs.length + " 个程序" : "当前视图 " + assets.length + " 个文件"}</span>
      ...
      {filter === "programs" && !trash ? (
        programsLoading ? (
          <div className="file-loading">正在读取程序库…</div>
        ) : programs.length === 0 ? (
          <div className="file-empty-state" aria-label="程序库为空">
            还没有程序——让小逻写个程序（游戏也行），成功后自动收进这里。
          </div>
        ) : (
          <div className="asset-grid file-grid">
            {programs.map((prog) => (
              <article key={prog.id} className="asset-card file-card">
                <button
                  type="button"
                  className="asset-preview file-preview"
                  onClick={() => openProgramDetail(prog)}
                  aria-label={"查看程序 " + prog.name}
                >
                  <span className="asset-glyph"><FolderOpen size={28} /></span>
                  <span className="asset-type">程序</span>
                  <span className="file-version">v{prog.version}</span>
                </button>
                <div className="asset-card-copy">
                  <div>
                    <h3 title={prog.name}>{prog.name}</h3>
                    <IconButton label="查看程序详情" onClick={() => openProgramDetail(prog)}>
                      <MoreHorizontal size={17} />
                    </IconButton>
                  </div>
                  <p>{prog.files.length} 个文件 · 入口 {prog.entry}</p>
                  <small>{formatDate(prog.updatedAt)}</small>
                </div>
                <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                  <button type="button" className="secondary-button" onClick={() => void renameProgram(prog)}>重命名</button>
                  <button type="button" className="danger-text-button" onClick={() => void removeProgram(prog)}><Trash2 size={15} /> 删除</button>
                </div>
              </article>
            ))}
          </div>
      ...
      {filter !== "programs" && nextCursor && !loading && (
        <button
          type="button"
      ...
      {filter !== "programs" && !loading && !visibleAssets.length && (
        <div className="file-empty-state" aria-label="当前没有文件" />
      )}

      {filter !== "programs" && loading && <div className="file-loading">正在读取文件索引…</div>}
      ...
      {programDetail && (
        <div className="preview-backdrop" onMouseDown={() => setProgramDetail(null)}>
          <aside
          ...
          >
            <div className="preview-drawer-heading">
              ...
              <IconButton label="关闭程序详情" onClick={() => setProgramDetail(null)}>
                <X size={18} />
              </IconButton>
            </div>
            <dl className="asset-details">
              <div><dt>入口文件</dt><dd>{programDetail.entry}</dd></div>
              <div><dt>版本</dt><dd>v{programDetail.version}</dd></div>
              <div><dt>更新时间</dt><dd>{formatDate(programDetail.updatedAt)}</dd></div>
            </dl>
            <div style={{ marginTop: 10, fontSize: 12, color: "#6b7280" }}>文件清单（点开看代码）</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 6, maxHeight: 340, overflowY: "auto" }}>
              {programDetail.contents.map((f) => (
                <details key={f.path} style={{ border: "1px solid #eef0f3", borderRadius: 8, padding: "6px 8px" }}>
                  <summary style={{ cursor: "pointer", fontSize: 12, color: "#374151" }}>{f.path} · {f.language}</summary>
                  <pre style={{ margin: "6px 0 0", fontSize: 11, lineHeight: 1.5, whiteSpace: "pre-wrap", overflowX: "auto" }}>{f.content}</pre>
                </details>
              ))}
            </div>
          </aside>
