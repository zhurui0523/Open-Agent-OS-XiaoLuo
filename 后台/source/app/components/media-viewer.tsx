...
export function MediaViewer({ url, kind, title, onClose }: MediaViewerProps) {
  ...
  return createPortal(
    <div
      ...
      aria-label={title}
      onClick={onClose}
      // Portal synthetic events bubble through the React tree (NodeCard -> canvas-stage),
      // so pointer input on this overlay must be stopped here, otherwise dragging the
      // zoomed image would pan the canvas underneath.
