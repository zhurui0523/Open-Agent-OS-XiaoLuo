...
export interface ChatMessage {
  role: ChatRole;
  content: string;
  /** 随消息直传模型的图片（多模态）：user 轮附件为图片时附带，模型可直接"看见" */
...
export interface ServicePort {
  ...
  /** 本地服务搭建（setup_service）：宿主负责批准弹框；成功返回连接串/地址 */
  setup?(payload: SetupServicePayload): Promise<SetupServiceResult>;
}
...
export interface ChatPersistence {
  ...
  interrupted?: boolean;
  /** 断点续跑（学 Codex session resume）：goal 模式目标跨刷新延续 */
  goal?: { goal: string; criteria: string; setAt: number };
  /** 断点续跑：本轮写改过的代码文件清单（改完必跑闸跨刷新延续，防验证断链） */
  dirtyCodeFiles?: string[];
  /** 断点续跑：本会话读过的文件清单（读后写纪律跨刷新延续） */
  readPaths?: string[];
  savedAt: number;
}
