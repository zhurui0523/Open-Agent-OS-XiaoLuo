...
export function useIntentOS() {
  ...
  const loadCanvasHome = useCallback(async () => {
    ...
    try {
      ...
      setCloudLoaded(true);
      // 重启恢复：优先回到最近使用的画布（对话按画布分区存档，回错画布会“对话消失”）
      void (async () => {
        try {
          const pref = await requestJson<{ preferences?: { lastCanvasId?: string } }>(
            "/api/v2/preferences",
          );
          const wanted = pref.preferences?.lastCanvasId;
          const hit =
            wanted && payload.canvases.some((c) => c.id === wanted)
              ? wanted
              : null;
          const fallback = [...payload.canvases].sort(
            (a, b) =>
              Date.parse(b.updatedAt || "0") - Date.parse(a.updatedAt || "0"),
          )[0]?.id;
          const target = hit ?? fallback;
          if (target && target !== payload.activeCanvas.id) {
    ...
    const timer = setTimeout(() => {
      saveSequence.current = saveSequence.current
        ...
        .then(async () => {
          ...
          const putPayload = (revision: number) =>
            requestJson<{ revision: number; savedAt: string }>(
              "/api/v2/canvases",
              {
                method: "PUT",
                headers: {
          ...
          try {
            let saved: { revision: number; savedAt: string };
            try {
              saved = await putPayload(draft.payload.revision);
            } catch (error) {
              // 单人使用却报冲突，多为本地 revision 落后（如切窗保存顶高了服务端版本）：
              // 自动拉最新 revision 重发一次，避免误报“其他位置更新”
              const conflictMessage =
                error instanceof Error ? error.message : "保存画布失败";
              if (!conflictMessage.includes("其他位置更新")) throw error;
              const latest = await requestJson<{
                canvas?: { revision?: number };
              }>(`/api/v2/canvases?id=${encodeURIComponent(canvasId)}`);
              const freshRevision = latest.canvas?.revision;
              if (typeof freshRevision !== "number") throw error;
              revisions.current.set(canvasId, freshRevision);
              saved = await putPayload(freshRevision);
            }
  ...
  useEffect(() => {
    const flushLatestCanvasSave = () => {
      ...
      void fetch("/api/v2/canvases", {
        ...
        .then(async (resp) => {
          if (!resp.ok) return;
          // 保存成功：把最新 revision 回写本地草稿与内存，避免下次保存带旧版本误报冲突
          const data = (await resp.json().catch(() => null)) as {
            revision?: number;
          } | null;
          if (typeof data?.revision !== "number") return;
          revisions.current.set(draft.payload.id, data.revision);
          if (latestCanvasSave.current?.snapshotKey === draft.snapshotKey) {
            latestCanvasSave.current = null;
          }
          writeCanvasDraft({
            ...draft,
            payload: { ...draft.payload, revision: data.revision },
            updatedAt: Date.now(),
          });
        })
        .catch(() => undefined);
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "hidden") {
        flushLatestCanvasSave();
      } else if (document.visibilityState === "visible") {
        // 切回窗口：把内存 revision 与本地草稿对齐（草稿 revision 只来自成功保存），
        // 防止内存落后服务端导致下一次保存误报冲突
        for (const canvasId of revisions.current.keys()) {
          const stored = readCanvasDraft(canvasId);
          const storedRevision = stored?.payload.revision;
          const inMemory = revisions.current.get(canvasId);
          if (
            typeof storedRevision === "number" &&
            typeof inMemory === "number" &&
            storedRevision > inMemory
          ) {
            revisions.current.set(canvasId, storedRevision);
          }
        }
      }
    };
  ...
  async function setActiveCanvasId(id: string) {
    ...
    await loadCanvas(id);
    // 记录最近画布：下次重启优先恢复（服务端合并保存，不覆盖其余偏好）
    void requestJson("/api/v2/preferences", {
      method: "PATCH",
      body: JSON.stringify({ preferences: { lastCanvasId: id } }),
    }).catch(() => {});
  }
      ...
      (preset.parameters?.source === "asset-kernel"
        ...
        : preset.parameters?.packageId && preset.parameters?.runtimeType
          ...
          : preset.parameters?.resultSlot === true
            ? "result"
            : "execution");
    // 所有新建节点 skill 默认「无」：只认显式指定，不做自动匹配
    const capability =
      inferredRole === "execution" && preset.capabilityId !== "none"
  ...
  function generateDirectly(
    ...
    // skill 默认「无」：仅用户显式选择时挂载
    const execCapability =
      capabilities.find((item) => item.id === rules.capability?.id) ?? null;
    const execModel =
      models.find(
        (item) =>
