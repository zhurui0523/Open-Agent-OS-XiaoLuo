...
// Mismatch fallback: sniff real format by magic bytes via hasExpectedSignature;
// only unambiguous strong signatures are adopted, others keep the rejection.
const SNIFF_MIME_TYPES = [
  "image/png",
  "image/jpeg",
  "image/gif",
  "image/webp",
...
function detectContentMimeType(bytes: Uint8Array): string | null {
  for (const candidate of SNIFF_MIME_TYPES) {
    if (hasExpectedSignature(candidate, bytes)) return candidate;
  }
  return null;
}
function withReplacedExtension(name: string, extension: string) {
  const index = name.lastIndexOf(".");
  return `${index > -1 ? name.slice(0, index) : name}.${extension}`;
}
...
export function validateUploadedFile(input: {
  ...
  let normalizedName = sanitizeAssetName(input.name);
  ...
  if (!hasExpectedSignature(mimeType, bytes)) {
    // Trust content when it sniffs as a supported format; rename extension.
    const detected = detectContentMimeType(bytes);
    const detectedFormat = detected ? formatForMimeType(detected) : null;
    if (!detectedFormat) {
      throw new Error("文件内容与扩展名不一致，已拒绝上传");
    }
    normalizedName = withReplacedExtension(
      normalizedName,
