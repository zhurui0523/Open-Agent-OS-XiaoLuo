...
function numericLayoutValue(
  node: CanvasNode,
  key: "layoutWidth" | "layoutHeight" | "mediaFitWidth",
) {
...
/** 媒体内容 contain 后的实际显示宽度（由卡片测量写回 parameters.mediaFitWidth）。
 *  卡片宽度与连线端点共用同一来源，保证连线球始终贴在内容边缘。 */
export function mediaFitWidthForNode(node: CanvasNode) {
  const value = numericLayoutValue(node, "mediaFitWidth");
  return value === undefined
    ? undefined
    : Math.min(MAX_NODE_WIDTH, Math.max(120, value));
}

export function widthForNode(node: CanvasNode) {
  const customWidth = customWidthForNode(node) ?? mediaFitWidthForNode(node);
