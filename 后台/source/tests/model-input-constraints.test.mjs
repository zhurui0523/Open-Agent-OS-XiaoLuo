...
test("professional text, image and video generation follows execution-node selection rules", () => {
  ...
  // 默认不自动挂技能：capability 为 null，模型不做协议过滤
  assert.equal(automatic.capability, null);
  assert.deepEqual(
    automatic.compatibleCapabilities.map((item) => item.id),
    ["skill.video.package"],
  );

  const withSkill = resolveProfessionalGeneratorRules({
    capabilities,
    models: [incompatible, compatible],
    kind: "video",
    requestedCapabilityId: "skill.video.package",
  });
  assert.equal(withSkill.capability?.id, "skill.video.package");
  assert.equal(withSkill.model?.id, "compatible");
