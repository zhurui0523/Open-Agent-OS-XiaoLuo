...
test("server-renders the connected XiaoLuoAgentOS shell", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>小逻Agent OS<\/title>/i);
  assert.match(html, /小逻Agent OS/);
  assert.match(html, /正在连接小逻Agent OS 云端内核/);
  assert.match(
    html,
