...
export async function listSandboxDir(
  ...
  const dir = resolveInWorkspace(userId, relPath);
...
export async function readSandboxFile(
  ...
  const file = resolveInWorkspace(userId, relPath);
...
export async function grepSandboxFiles(
  ...
  const root = resolveInWorkspace(userId, opts?.path || ".");
...
// ---------- run_command 风险分级（一期审批门：放行 / 需批准 / 硬拦截） ----------

export type CommandRiskLevel = "safe" | "needs_approval" | "blocked";

/** 硬拦截签名：不可逆 / 系统级 / 越权操作（fail-closed，命中即拒） */
const COMMAND_BLOCK_PATTERNS: Array<{ label: string; pattern: RegExp }> = [
...
/** 需批准签名：装包等副作用较大但可恢复的操作（老板确认后放行） */
const COMMAND_APPROVAL_PATTERNS: Array<{ label: string; pattern: RegExp }> = [
...
/**
 * 命令轻量语法分析（tree-sitter-bash 的零依赖替代，仅用于风险签名匹配、不参与执行）：
 * 1. 抽出命令替换 $(...) 与反引号内容作为独立扫描段（防危险命令藏在替换里）；
 * 2. 引号串内容置占位（引号包住的关键词只是"提到"不构成执行意图，消除 echo "rm -rf" 类误杀），
 *    例外：sh -c / powershell -Command 等解释器参数后的引号内容照扫（那是真会被执行的）。
 * 与 desktop/local-runtime.mjs 的同名函数保持同步维护。
 */
function normalizeCommandForRisk(command: string): string {
  ...
  const stripQuotes = (seg: string): string => {
    let out = "";
    let i = 0;
    while (i < seg.length) {
      const ch = seg[i];
...
/** 裸命令分级（run_command 用；代码内容扫描另见 xiaoluo-brain/lib/brain/sandbox.ts） */
export function classifyCommandRisk(command: string): { level: CommandRiskLevel; reasons: string[] } {
  // 先过轻量语法分析：引号内提及的关键词不再误杀，命令替换里的内容照扫
  const scan = normalizeCommandForRisk(command);
  const reasons: string[] = [];
  for (const rule of COMMAND_BLOCK_PATTERNS) {
    if (rule.pattern.test(scan)) reasons.push(rule.label);
  }
  if (reasons.length > 0) return { level: "blocked", reasons };
  for (const rule of COMMAND_APPROVAL_PATTERNS) {
    if (rule.pattern.test(scan)) reasons.push(rule.label);
  }
...
function appendServiceLog(rec: ServiceRecord, chunk: Buffer): void {
  rec.log = (rec.log + chunk.toString("utf8")).slice(-MAX_SERVICE_LOG_CHARS);
