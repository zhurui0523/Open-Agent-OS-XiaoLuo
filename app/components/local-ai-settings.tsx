...
export function LocalAiSettings({
  ...
  useEffect(() => {
    ...
    const off = bridge?.onLocalAiEvent?.((event) => {
      if (event.modelId) {
        setProgress((prev) => ({ ...prev, [event.modelId as string]: event }));
      }
      void refresh();
    });
    return () => {
  ...
  function modelProgress(modelId: string) {
    ...
    if (!event || !event.receivedBytes || !event.totalBytes) return null;
    return {
      percent: Math.min(100, Math.round((event.receivedBytes / event.totalBytes) * 100)),
      ...
      <div className="local-ai-model-row" key={model.id}>
        <div className="local-ai-model-info">
          ...
          <span className="local-ai-hint">
            ...
            {" · 建议 " + (tierLabels[model.minTier] ?? model.minTier)}
          </span>
          ...
          {!model.installed && !prog && (
            <button
              ...
              onClick={() => {
                ...
                void act("download-model", { modelId: model.id }, "开始下载 " + model.name);
              }}
            >
              <Download size={13} /> 下载
            </button>
    ...
    return (
      <div className="local-ai-page">
        ...
        <div className="local-ai-card">
          ...
          <p className="local-ai-hint">
            覆盖文本/图片/视频七类开源模型；支持断点续传、多文件编排与完整性校验，不满足硬件门禁的模型会标注原因。
          </p>
          {renderModelLibrary()}
        </div>
