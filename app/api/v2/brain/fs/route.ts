...
export async function POST(request: Request) {
  try {
    ...
    if (action === "list") {
      const entries = await listSandboxDir(user.id, relPath);
      return Response.json({ entries });
    }
    if (action === "read") {
      const win = await readSandboxFile(user.id, relPath, body.opts);
      return Response.json(win);
    }
    if (action === "grep") {
      const hits = await grepSandboxFiles(user.id, relPath, body.opts);
      return Response.json({ hits });
    }
    if (action === "find") {
      const paths = await findSandboxFiles(user.id, relPath, body.opts);
      return Response.json({ paths });
    }
    if (typeof body.content !== "string") {
      return Response.json({ error: "write 需要 content 字符串" }, { status: 400 });
    }
    await writeSandboxFile(user.id, relPath, body.content, body.opts?.encoding === "base64" ? { encoding: "base64" } : undefined);
